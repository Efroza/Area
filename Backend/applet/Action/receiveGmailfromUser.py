...
mandatory = ['email', 'from']
name = 'Receive Gmail Fom Specified User'
description = 'Active Applet when receive a mail in your gmail account from specified user'
service = 'Google'
...
def getGmailFromUser(mail : imaplib.IMAP4_SSL, fromm : str):
    now = datetime.now()
    return mail.search(None, "(SINCE " + now.strftime("%d-%b-%Y") + ' FROM ' + fromm + ')')
...
def main(id_user : int, query : QueryDict) -> bool:
    ...
    if user == None or password == None or connectGmail(mail, user, password) == False:
        return ERROR
    result, data = getGmailFromUser(mail, fromm)
